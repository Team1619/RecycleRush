...
public class ManualKeyboardControl {
	
	final private static ManualKeyboardControl highlander = new ManualKeyboardControl();
	private ManualKeyboardControl() {}
	public static ManualKeyboardControl getInstance() {
		return highlander;
	}
	...
	public void startRainbowSTORMServer() {
		new Thread(new Runnable() {
			...
			public void run() {
				...
				while(true) {
					try {
						serverSocket = new ServerSocket(RobotMap.rainbowSTORMPort);
						while(true) {
	...
	public void runRainbowSTORM() {
		...
		for(RobotMap.MotorDefinition md : RobotMap.MotorDefinition.values()) {
			...
			motor.set(forwardKey ?
					reverseKey ? 0.0 : md.forwardKeySpeed :
