...
public class UXTR3M3BinRakerMkIIN0Sc0peByMountainDew extends CommandGroup {
    ...
    public  UXTR3M3BinRakerMkIIN0Sc0peByMountainDew() {
    	...
    	// Slow down...
    	addSequential(new ULinearDriveCommand(UPreferences.getPreferences().get("InitialSlowDriveDistance", 1.0), 0.5));
    	
    	// Grab the bin
    	addParallel(new UClawsClose(0.5, 2.0));
    	
    	// Drive over bin as it's grabbed
    	addSequential(new ULinearDriveCommand(UPreferences.getPreferences().get("SecondSlowDriveDistance", 1.0), 0.5));
    	...
    	// Shake bins loose
    	addSequential(new ULinearDriveCommand(UPreferences.getPreferences().get("BackDriveShakeDistance", -0.1), 0.7));
    	addSequential(new ULinearDriveCommand(UPreferences.getPreferences().get("ForwardDriveShakeDistance", 0.2), 0.7));
